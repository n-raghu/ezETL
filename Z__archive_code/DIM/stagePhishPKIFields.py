...
def pushChunk(p_id, prime_keys, pgURI, ins_code, tabSchema, pgTable, db_table, chk, model_traker):
    ...
    try:
        ...
        logError(
            pid,
            f'stagePhishPKIFields',
            f'PushChunkError| {str(pgTable)}| {str(err)}',
            pgURI
        )
        ...
        if errType in [
            ...
            try:
                ...
                logError(
                    pid,
                    f'stagePhishPKIFields',
                    f'ExceptionDataFrameError| {str(pgTable)}| {str(unknownError)}',
                    pgURI
                )
    csv_dat = None
    del chk
...
@r.remote
def popCollections(pid, one_ins, model_trk, iFrame):
    pgx = pgcnx(uri)
    icode = one_ins['icode']
    ...
    try:
        sqx = mysqlCNX(
            user=one_ins['user'],
            passwd=one_ins['password'],
            port=int(one_ins['port']),
            db=one_ins['database'],
            host=one_ins['host'],
            use_pure=False
        )
    ...
    for idx, rowdata in iFrame.iterrows():
        ...
        sql = f'SELECT {primeKeys} FROM {rco}'
        allFrames = []
        try:
            chunk = rsq(sql, sqx,)
            ...
            if False in allFramesResponse:
                ...
                nuSession = dataSession(uri)
                nuSession.execute(f"UPDATE framework.cachetraces SET status=true WHERE pid={str(pid)} AND collection='{cachecollection}'")
                nuSession.commit()
                nuSession.close()
        except Exception as err:
            logError(
...
print(f'Active Instances Found: {str(len(insList))}')
