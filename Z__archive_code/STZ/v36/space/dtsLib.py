...
try:
	...
	from copy import *
	import yaml
	from pandas import DataFrame,read_sql,options,Series,read_sql_query
...
class eaeFuse(eaeAdapter):
	def __init__(mem):
		...
		mem.cacheStudents=mem.mongoConnect[mem.eae].cache_studentData
	def eaeObjects(mem):
		eaeObj=odict()
		eaeObj['log_error']=mem.dbx
		eaeObj['log_forker']=mem.dbf
		eaeObj['cstat']=mem.dbs
		eaeObj['change_tracker']=mem.dbc
		eaeObj['student_data']=mem.studentData
		eaeObj['camp_data']=mem.phishCampData
		eaeObj['dash_data']=mem.phishDashboardData
		eaeObj['dash_params']=mem.phishDashboardParams
		eaeObj['cache_students']=mem.cacheStudents
		return eaeObj
...
class appFuse(eaeFuse):
	...
	def lmsFuse(mem):
		...
		for ins in mem.lmsInsList:
			...
			lmsConnections.append(odict([('ins',ins['instancename']),('icode',ins['instancecode']),('cnx',sqlConStr)]))
