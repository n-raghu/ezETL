...
for idx,rowdata in colFrame.iterrows():
	...
	icode=rowdata['icode']
	primeKeys=colFrame.loc[(colFrame['collection']==rco)]['pki_cols'].values.tolist()[0]
	sql="SELECT " +primeKeys+ " FROM " +rco
	print(sql)
	chunk=rsq(sql,cnx)
	chunk['instancecode']=icode
	chunk.to_sql(cachecollection,pgx,if_exists='append',index=False,schema='staging')
